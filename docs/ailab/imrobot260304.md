...
3. 在 Jetson 开发板启动 **终端 Terminal** App [^1]，在终端中执行 `cheese` 命令，可在弹出的窗口中看到图像，即表明摄像头能正常工作。
...
❇️ 建议：如果还不大熟悉 Linux 命令和操作，尽量手敲以尽快熟悉。<br>
✨ 提示：浏览器和其他图形界面中，复制/粘贴的快捷键是 ctrl + c 和 ctrl + v。<br>
✨ 提示：在 **终端 Terminal** App 中，复制/粘贴的快捷键是 ctrl + shift + c 和 ctrl + shift + v。

- 启动浏览器访问网站

    在 Jetson 开发板上启动 **Firefox 浏览器** App [^1]，访问 `https://tnt.gdvzz.com/`，找到本次实验的文档：AI Lab → AI实验课 → 视觉实验-260304。

- 在 Jetson 开发板上启动 **终端 Terminal** App[^1]： 在 Ubuntu 主界面左侧导航栏找到对应的 App，或者在顶部搜索栏输入 App 的名字查找。  
...
在 Jetson 开发板上启动 **终端 Terminal** App[^1]，然后输入 `conda` 并回车。执行后如果屏幕显示“命令没有找到”之类的信息，则表明尚未安装，请参考如下命令先安装 Conda。如果已经安装 Conda，请跳到第2步。
...
✨ 提示：在终端上输入命令时可用 `tab` 键补齐文字，以加快输入[^4]。
