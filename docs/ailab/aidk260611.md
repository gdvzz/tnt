...
mkdir ~/oss0611
    ...
    > （1）该目录的完整路径是：/home/HwHiAiUser/oss0611<br>
...
4. **上传压缩包到开发板的实验目录中**
    ...
    ```bash
scp Pytorch-MobileFaceNet-master-V2.zip HwHiAiUser@192.168.137.100:/home/HwHiAiUser/oss0611
    ```
...
unzip Pytorch-MobileFaceNet-master-V2.zip
    ...
    解压缩后生成子目录 Pytorch-MobileFaceNet-master-V2，完整路径应该是：/home/HwHiAiUser/oss0611/Pytorch-MobileFaceNet-master-V2。
...
1. **创建 conda 虚拟环境**
    ...
    ```bash
conda create -n eoss0611 python=3.11
    ```
...
4. **安装其他依赖库**
    ...
    ```bash
cd ~/oss0611/Pytorch-MobileFaceNet-master-V2
    ```
...
- ✴️ Conda（Python）虚拟环境（本文名称样例是 eoss0611），创建一次即可。不需要反复重复创建。
...
...
    ...
    ✳️ 可以用 PC（个人电脑）上 VSCode远程连接开发板后修改代码。详见 [VSCode指南↗]
