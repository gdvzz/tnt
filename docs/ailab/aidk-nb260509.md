...
**关于朴素贝叶斯（NB, Naive Bayes）**

本次实验将使用 **朴素贝叶斯** 实现 MINST 分类，即将原始 MNIST 图像的 灰度像素值 通过一个阈值（通常为 127 或 0.5 归一化后）转化为 0/1 二值特征，然后使用 **伯努利朴素贝叶斯（BernoulliNB）** 模型进行分类。

朴素贝叶斯基于贝叶斯定理，并假设 **特征之间条件独立**。对于二值特征，每个类别的似然函数采用 **伯努利分布**：

```text
P(x|C_k) = ∏_{i=1}^{784} [ p_{ki}^{x_i} · (1-p_{ki})^{1-x_i} ]
```
...
- **条件概率** `p_{ki}` = (类别 k 中第 i 个像素为 1 的样本数 + α) / (类别 k 的样本数 + 2α) `（α 是拉普拉斯平滑参数，默认 α=1.0，避免零概率）`
...
4. **在虚拟环境中安装相关包：**
    ...
    ```bash
    pip3 install numpy scikit-learn pillow joblib
    ```

    ✳️ 亦可用 conda 安装，效果是一样的。命令是 `conda install numpy scikit-learn pillow joblib`。

<!-- 对应 train3.py 和infer3.py -->
...
- 训练结果保存为模型。
- MNIST的4个数据 gz 文件已下载，放在 ./data/MNIST/raw 目录下
- 如果 gz 文件不存在，就报错

3、输出推理程序。

- 图片是黑字白底。
- 可以对单张图片做推理。图片路径作为命令行传入。图片名称（除扩展名）的最后一位是0～9，表示该图片是数字0～9之一。
- 还可以对一个目录下所有图片做推理。图片名称（除扩展名）的最后一位是0～9，表示该图片是数字0～9之一。
- 输出哪些文件推理正确，哪些错误。 
- infer.py 增加个功能（可用命令行参数控制开关）：输出处理后的 28*28的图片 
-->
