...
本次实验将使用 **朴素贝叶斯** 实现 MNIST 分类，即将原始 MNIST 图像的 灰度像素值 通过一个阈值（通常为 127 或 0.5 归一化后）转化为 0/1 二值特征，然后使用 **伯努利朴素贝叶斯（BernoulliNB）** 模型进行分类。
...
4. **在虚拟环境中安装相关包：**
    ...
    ✳️ 亦可用 conda 安装，效果是一样的。命令是 `conda nstall numpy Pillow flask opencv-python`。
...
1. **开发板上启动 Web 服务端**
    ...
    * Debugger PIN: 921-919-078
    ...
    ✳️ 如果打不开摄像头，可参考屏幕提示信息，把用户 `HwHiAiUser` 添加到 Linux 的 `video` 组，就可以打开摄像头了。
