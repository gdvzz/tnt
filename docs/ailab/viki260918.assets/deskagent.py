...
"""
桌面小管家 Agent - 演示收尾版
Jetson 环境感知 + 本地 Ollama 生活助手

优化点：
1. 工具返回短文本，减少 prefill
2. get_room_status() 组合工具，一次拿全环境数据
3. 精简 system prompt 和工具 schema
4. 摄像头句柄复用
5. /fast 模式：跳过第一轮 LLM
6. 内置样例问题菜单，支持编号快捷输入
7. num_predict=120，避免回答截断
"""
...
PHOTO_DIR = "/tmp/desktop_agent_photos"
MAX_TOOL_ROUNDS = 2
KEEP_ALIVE = "30m"
LLM_OPTIONS = {
    "num_predict": 120,
...
SAMPLES = [
    ("看看现在房间环境怎么样，给我一点居家建议", "环境总览（调 get_room_status）"),
    ("外面光线够不够，要不要开台灯？", "亮度判断（调 capture_light_estimate）"),
    ("帮我拍张照", "拍照（调 take_photo）"),
    ("现在温度和湿度是多少？", "温湿度（调 get_temphum）"),
    ("请自我介绍", "不调工具，直接回答"),
]
...
def main():
    print(BANNER)
    cprint(
        f"[配置] 模型: {OLLAMA_MODEL} | 摄像头: {CAMERA_INDEX} | 真实BME280: {USE_REAL_BME280}",
        C.DIM,
    )
    warmup()
    print_samples()
