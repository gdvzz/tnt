...
1. **创建 conda 虚拟环境**
    ...
    ```bash
conda create -n cnne0604 python=3.10
    ```
...
- ✴️ Conda（Python）虚拟环境（本文名称样例是 cnne0604），创建一次即可。不需要反复重复创建。
...
mkdir ~/mcnn0604
    ...
    该实验目录的完整路径是：/home/HwHiAiUser/mcnn0604
...
3. **上传压缩包到开发板的实验目录中**
    ...
    **方式二：** 或者在本地电脑敲命令传文件。请参考：[Linux常用操作↗](https://tnt.gdvzz.com/aikit/linuxug.html) \| scp 远程复制文件/目录。比如：
    ...
    ```bash
scp cnn260519.zip HwHiAiUser@192.168.137.100:/home/HwHiAiUser/mcnn0604
    ```
...
unzip cnn260519.zip
    ...
    解压缩后生成子目录 mnist-master，完整路径应该是：/home/HwHiAiUser/mcnn0604/mnist-master。
...
1. **先进入样例代码目录**
    ...
    ```bash
cd ~/mcnn0604/mnist-master
    ```

2. **并确保虚拟环境已激活**

    命令行提示符首部有 `(cnne0604)` 字样，即表示本次实验用虚拟环境已激活。如需激活，可执行以下命令：
...
关于普通用户使用摄像头，可参考：[鲲鹏开发板指南-摄像头↗]（适用于鲲鹏开发板和昇腾开发板）
...
![cnn1.jpg](./aidk260604.assets/cnn1.jpg)

相关参考：

- [try_tts.py](./aidk260604.assets/try_tts.py)：测试声音。
- [鲲鹏开发板指南-喇叭和麦克风↗]：适用鲲鹏和昇腾开发板。

<details markdown="block">
  <summary>参考样例代码</summary>

- [webapp.py](./aidk260604.assets/webapp.py)
...
[鲲鹏开发板指南-摄像头↗]: https://tnt.gdvzz.com/aikit/dkoo.html#access-camera
