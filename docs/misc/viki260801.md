...
1. **创建虚拟环境**
    ...
    ```bash
conda create -n irbt python=3.9 pip
    ```
...
2. **抓取 blue（蓝色）积木**
    ...
    ```bash
grab blue cube and move to -60,222,110
    ```
...
3. **抓取 green（绿色）积木**
    ...
    ```bash
grab green cube and move to 10,222,110
    ```
...
4. **抓取 red（红色）积木**
    ...
    ```bash
grab red cube and move to 80,222,110
    ```
...
5. **抓取 yellow（黄色）积木**
    ...
    ```bash
grab yellow cube and move to 140,222,110
    ```
...
4. **粘贴**
    ...
    - 按 `i` 键
    - 按 `Ctrl` `Shift` `V`（3个键同时按下）。或者鼠标右键 → Paste
    - 按 `Esc` 键（在键盘的左上角）
    - 输入 `:wq` 后，按 `Enter(回车)` 键
