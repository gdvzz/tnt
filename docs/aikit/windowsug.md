...
- 如要求设置 **网关（Gateway）**，则输入`192.168.137.1`。网关信息不会用到。再尝试点击 **保存（Save）** 按钮
...
在 PC（个人电脑）执行 ping 命令，测试是否可以通过网络（网线）连通开发板。

1. **在PC（个人电脑）启动终端 PowerShell**

    按 `win` + `s` 键 → 搜索框输入 `powershell` → 点击运行找到的 powershell

2. **在终端 PowerShell 执行 ping 命令**

    ```bash
ping 192.168.137.100
    ```

    如能看到以下类似的提示信息，则表明 PC（个人电脑）和开发板之间的网络连通了。

    ```bash
...
Pinging 192.168.137.100 with 32 bytes of data:
Reply from 192.168.137.100: bytes=32 time<1ms TTL=64
Reply from 192.168.137.100: bytes=32 time<1ms TTL=64
Reply from 192.168.137.100: bytes=32 time<1ms TTL=64
Reply from 192.168.137.100: bytes=32 time<1ms TTL=64
...
    ...
    [![pingdk](./windowsug.assets/pingdk.png)](./windowsug.assets/pingdk.png)

**✳️ 如果不能 ping 通，可检查：**

- **是否设置了 PC（个人电脑）的正确的以太网**。要设置连接开发板的以太网，不能设置不是连接开发板的以太网
- **是否设置了 PC（个人电脑）的正确的IP 地址**。PC（个人电脑）的 IP 不能设置为 `192.168.137.100`，因为该地址是开发板的 IP 地址。
- **网线的两端是否插紧**。网线不能“塞”到网口中，要插紧，能听到“喀哒”的声音。网线连接正常时间，可观察有指示灯的网口，应该绿灯常亮 + 黄灯闪烁。
- **昇腾开发板的网线是否连接正确**。昇腾开发板有上下排列2个网口，要连接上面那个网口。
- **开发板是否已启动完成**。可能误碰了开发板的关机键，开发板虽连着电源但已关机。
