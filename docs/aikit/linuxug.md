...
- **复制/粘贴一行**
    ...
    光标移到待复制行 → 按 `Esc`  → 按 `yy` → 光标移到目标位置 → 按 `p` 复制到下一行 （按 `P` 复制到上一行）

- **往后查找**

    按 `Esc`  → 按 `/` → 输入要查找的内容 → 按 `Enter回车`

    找到后，按 `n` 到下一个（往后）；按 `N` 到上一个（往前）

- **往前查找**

    按 `Esc`  → 按 `?` → 输入要查找的内容 → 按 `Enter回车`

    找到后，按 `n` 到下一个（往前）；按 `N` 到上一个（往后）

- **保存**

    按 `Esc`  → 输入 `:w` → 按 `Enter回车`
